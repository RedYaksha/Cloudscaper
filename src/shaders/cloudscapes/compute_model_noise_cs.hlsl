...
void main( ComputeShaderInput IN ) {
    ...
    const float perlin_fbm = PerlinFBM(coord);
    float perlin_worley = 0.f;
    {
        static const float worley_freq_mults[] = {2.f, 8.f, 14.f, 20.f, 26.f, 32.f};
        const float worley0 = (1.f - Worley(coord, cell_count * worley_freq_mults[0]));
        const float worley1 = (1.f - Worley(coord, cell_count * worley_freq_mults[1]));
        const float worley2 = (1.f - Worley(coord, cell_count * worley_freq_mults[2]));

        // fbm, increasing frequency, decreasing amplitude
        const float worley_fbm = worley0 * 0.625f + worley1 * 0.25f + worley2 * 0.125f;

        //perlin_worley = Remap(worley_fbm, 0.0f, 1.0f, 0.0f, perlin_fbm);
        perlin_worley = Remap(perlin_fbm, 0.0f, 1.0f, worley_fbm, 1.);
        //perlin_worley = Remap(perlin_fbm, 1 - worley_fbm, 1.0f, 0.0f, 1.0f);
    }
    ...
    float4 Output = float4(perlin_worley, worley_fbm0, worley_fbm1, worley_fbm2);
